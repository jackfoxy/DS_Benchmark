...
module NaiveFlatList =
    ...
    let getTime (inputArgs:BenchArgs) data =
        ...
        | x when x = Action.AddOne -> 
            Utility.timeAction (Seq.fold (fun (q : 'a FlatList) t -> FlatList.append q (FlatList.one t)) FlatList.empty) data (sprintf "%s %s" Operator.SeqFold Operator.Conj)
